...
def get_namespaces_by_label(json_data, label_key, label_value=None):
    """
    Extract namespace names associated with a specific label from the provided JSON data.
    ...
    Returns:
        list: A list of namespace names matching the label criteria.
    """
    namespaces = []
    ...
    for item in json_data.get("items", []):
        ...
        if label_key in labels and (label_value is None or labels[label_key] == label_value):
            namespaces.append(metadata.get("name"))
...
print("Namespaces with the specified label:", namespaces)
